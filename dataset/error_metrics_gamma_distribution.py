...
for idx, structure_data in enumerate(data['data_points']):
    ...
    # Compute gamma values for each atom
    ...
    if gamma_values is not None:
        ...
        if isinstance(gamma_values, (list, np.ndarray)):
            ...
            if len(gamma_values) == num_atoms:
                all_gammas.extend(gamma_values)
                for i, gamma in enumerate(gamma_values):
                    print(f"Structure {idx+1}, Atom {i+1}: Gamma = {gamma:.4f}")
            else:
...
# Plotting gamma values distribution
plt.figure(figsize=(10, 6))
plt.hist(all_gammas, bins=50, color='blue', edgecolor='black')
